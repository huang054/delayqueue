...
public class OrderOverTimeClose {
     ...
     * 守护线程
     */
    private Thread mainThread = new Thread(() -> execute());
    ...
    private void execute() {
        ...
        while (true) {
            try {
                if(queue.size() > 0){
                    //从队列里获取超时的订单
                    OrderInfo orderInfo = queue.peek();
                    ...
                    if (Objects.equals(orderInfo.getStatus(), "成功")) {
                       queue.poll();
                        ...
                        Thread.sleep(2000);
                    } else if(Objects.equals(orderInfo.getStatus(), "订单初始化")){
                        try {
                            if(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(orderInfo.getExpTime()).getTime()<System.currentTimeMillis()) {
                                queue.poll();
                                System.out.println("线程：" + Thread.currentThread().getName() + "，订单号："
